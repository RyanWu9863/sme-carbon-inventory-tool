# 進度與決策紀錄

換一台電腦接著做時，先讀這一份。程式怎麼裝、怎麼跑在 [README.md](README.md)。

最後更新：2026-08-12（種子資料已進資料庫，115 個測試全綠）

---

## 換電腦怎麼開始

```bash
git clone https://github.com/RyanWu9863/sme-carbon-inventory-tool.git
cd sme-carbon-inventory-tool
python -m venv .venv && .venv\Scripts\activate (PowerShell: Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope Process; .venv\Scripts\Activate.ps1)
pip install -r requirements.txt
python -m pytest tests/ -q          # 應該看到 115 passed
python scripts/import_seed.py       # 建 carbon.db 並匯入種子資料
```

**兩個檔案不在版控裡**（USB 或雲端硬碟，不要傳進 repo）：

| 檔案 | 為什麼不在 repo | 少了它會怎樣 |
|---|---|---|
| `專題執行計畫_v2.xlsx` | 含風險清單與砍功能順序，是內部判斷，repo 是 public | 沒差，這份文件的「路線圖」一節已經取代它 |
| `溫室氣體排放量清冊表單(範例).ods` | 916 KB 官方原檔，且內嵌原始製表者的內部路徑 | **沒差了**，代碼表已抽成 CSV 進版控 |

**兩個都不再擋路。** 上一版寫「.ods 少了下一步做不了」，那件事已經做完 —— 代碼表
7,603 筆在 `data/codes/`，115 個測試在沒有 .ods 的機器上照樣全綠。.ods 只有官方
改版、要重抽代碼表時才需要。

---

## 專案定位

**自主練習作品，沒有指導老師、沒有評分。**（2026-08-12 確認）

這件事改變的東西寫在下面「路線圖調整」。**沒有改變的是揭露紀律**：v5 試算表裡那些「口試會問」的限制說明照樣要寫。沒有人來問，不代表數字可以不交代來源——那正是這個練習在練的東西。

---

## 現在做到哪

**驗收基準達成：從資料庫算出 7.531736 tCO2e。**

```
python scripts/import_seed.py    官方係數與代碼表
python scripts/load_demo.py      示範小吃店
python scripts/calc_demo.py      → 7.531736 tCO2e
```

整條鏈打通了：ORM 查係數 → 推導每單位係數 → 跨期分攤 → 計算 → 彙總表八。

| 元件 | 狀態 |
|---|---|
| `app/models.py` — 13 張表，對齊官方表一~表七、附表一/二/五~七 | ✅ |
| `app/calculator.py` — 熱值換算、GWP 加總、跨期分攤、完整性檢查 | ✅ |
| `app/seed.py` — 從 v5 試算表讀係數／熱值／電力／GWP／公告出處 | ✅ |
| `app/service.py` — 查係數、算一筆、算年度 | ✅ |
| `scripts/extract_codes.py` — 官方 .ods → `data/codes/*.csv` | ✅ |
| `scripts/import_seed.py` — 代碼表＋係數 → `carbon.db`，可重複執行 | ✅ |
| `scripts/load_demo.py` — 示範案例，選配 | ✅ |
| `scripts/calc_demo.py` — 算年度並印出表八 | ✅ |
| `app/api.py` — 10 個端點，附 Swagger UI | ✅ |
| `app/static/index.html` — 三個畫面，單一檔案無 build step | ✅ |
| `tests/` — 115 個測試，全綠 | ✅ |
| 報表與 Excel 匯出 | ❌ **下一步** |
| 使用者測試（找同學／家人） | ❌ 不可砍 |
| demo 影片 | ❌ 不可砍 |

**7.531736 現在有兩條獨立路徑算得出來**：純函式
（`test_totals_match_spreadsheet_table8`）與資料庫
（`test_year_total_matches_spreadsheet_table8`），加上 Excel 公式本身共三條。

---

## 這次（2026-08-11 ~ 12）做了什麼

### 上傳 GitHub

<https://github.com/RyanWu9863/sme-carbon-inventory-tool>（public）

上傳前檢查過 v5 試算表：裡面是示範資料不是真實單據（「示範小吃店」、電號 `01-23-4567-89`、車牌 `3888-AB` 都是假的，統編與地址是「待填」），程式端沒有金鑰，`db.py` 走 `CARBON_DB_URL` 環境變數。

`tests/` 資料夾原本叫 `test/`，README 寫的卻是 `tests/`，照 README 打指令會失敗。已改成 `tests/`。

### 修正 v5 的一處自相矛盾

「資料來源與限制」第 5 條原本寫「本表燃料類一律採 30」，與第 0 條、GWP表分頁、程式、測試四處採用的 28 衝突——是平台公告修正前的殘留文字。已改寫。

**後來下載的官方表單正好背書**：2026/6/1 修正版的「表五 修正說明」明載，依平台公告，燃料燃燒（含固定及移動設備）採「甲烷」GWP=28。報告裡可以直接引這一條，不必說「我猜 28」。

改法是只替換 `xl/sharedStrings.xml` 裡那一條字串，其餘 zip entry 原樣複製——用 openpyxl 重寫整個工作簿會丟掉公式的快取值，GitHub 預覽會變成一片空白。

### 種子資料從試算表讀，不手抄

`app/seed.py` + `tests/test_seed.py`（新增 11 個測試）。

**為什麼不手抄成 Python 常數**：手抄會產生第二份真相。係數改版時要記得改兩個地方，漏掉一個，程式與試算表就開始各說各話，而且兩邊都「正常執行」，只是答案不同。

附帶好處是這張比對表——試算表 U 欄是 Excel 公式算的，左欄是 Python 算的，兩條路徑獨立：

```
係數編號                        程式重算            試算表        差
TW-F-GASOLINE-S         2.189542995648    2.189542995648   0.0e+00
TW-F-DIESEL-S           2.685856261540    2.685856261540   4.4e-16
TW-F-NG-S               1.906017804484    1.906017804484   2.2e-16
TW-M-GASOLINE-OXI       2.270679160320    2.270679160320   0.0e+00
...（共 12 個，最大差 4.4e-16）
```

原本 `test_calculator.py` 只手抄 5 個燃料進測試檔，現在 12 個全涵蓋，而且在試算表新增燃料時會自動跟著測。

三個刻意的設計：

1. **欄位搬動擋在門口**（`_check_header`）。讀錯欄位的種子資料看起來完全正常——熱值欄讀成 CO2 欄會得到一組合理但全錯的係數，沒有任何錯誤訊息。有測試故意對調欄位確認擋得住。
2. **114 年度電力係數尚未公告，不讓它靜靜消失。** 跳過該列，但年度記在 `electricity_pending_years`。否則盤查 114 年度只會得到「查無係數」，沒人知道是因為政府還沒公告。
3. **試算表沒算過公式時明確報錯。** `data_only=True` 讀的是 Excel 存檔時的快取值；檔案沒被 Excel 開過的話 U 欄會是 `None`，此時直接告知「用 Excel 開啟後存檔再匯入」。

### 官方清冊表單已下載並驗證

`溫室氣體排放量清冊表單(範例).ods`，2026/6/1 修正版。

- 附表五 製程 **1,024** 筆、附表六 設備 **359** 筆、附表七 原燃物料 **6,256** 筆，與計畫表數字完全一致
- 結構是 `序號 | 代碼 | 名稱`，可直接匯入
- 另一個有用的修正說明：表三的外購台電電力代碼更新為 `GG3500`——v5 用的就是 GG3500

### 代碼表抽成 CSV，.ods 從此不必再帶著跑

`scripts/extract_codes.py` → `data/codes/*.csv` + `tests/test_codes.py`（新增 16 個測試）。

**三個發現，跟計畫表記的數字都不一樣：**

1. **筆數少 36 筆。** 原本記的 1,024 / 359 / 6,256 是**含表頭的列數**，實際資料列是
   1,023 / 358 / 6,255。照 7,639 寫驗收測試會直接失敗。
2. **附表七有 33 筆重複。** 官方把序號 6179~6211 那一段（廢液與污染土壤，
   `R-25xx`／`S-01xx`／`S-02xx`）原封不動複製成 6223~6255。`MaterialCode.code`
   是主鍵，不處理會直接擋住匯入。逐筆比對過，33 組全是同代碼同名稱，零衝突。
3. **v5 手挑的 23 筆代碼與官方全量逐字一致**（含 `GG3500`）。代表現有的
   `material_code` 外鍵不會斷，不需要資料遷移。已寫成測試釘住。

去重後 **1,023 + 358 + 6,222 = 7,603 筆**。這才是匯入的驗收基準。

**三個刻意的設計：**

1. **不裝 odfpy／pandas，自己解 XML。** 這支腳本一輩子只跑幾次，卻會讓每個 clone
   的人都多裝一個套件。ODS 就是 zip 裝著 `content.xml`，標準函式庫夠用。

2. **ODS 的儲存格壓縮是會安靜出錯的地方。** ODS 把連續相同的儲存格壓成一個，用
   `number-columns-repeated` 記次數，一列尾端常是「一個空格重複 16384 次」。照實
   展開會炸記憶體；無視 repeat 則會**讓欄位左移，讀出一份看起來完全正常的錯資料**。
   作法是照實展開但取滿三欄就停。

3. **序號連續性檢查，用官方序號當校驗碼。** 這是整支腳本最重要的一道。解析錯誤不會
   拋例外，6,255 讀成 6,250 你不會發現。官方序號本來就是 1..N 連續，驗證它，就不必
   事先知道正確筆數也能證明沒讀漏 —— 一道同時擋掉漏讀、重複讀、順序錯亂。

去重規則是**同代碼同名稱才刪，同代碼不同名稱一律拋錯**。後者現在一筆都沒有，但真
出現時代表官方改了名稱或代碼被重用，該由人判斷；程式替你挑一個，等於把一個需要決策
的問題變成一個看不見的問題。被移除的 33 筆連同它跟哪一筆重複，都記在
`data/codes/README.md`。

測試刻意設計成**不需要 .ods**：去重邏輯用自編假資料測，其餘測已進版控的 CSV。所以
任何一台 clone 這個 repo 的電腦都跑得起來 —— 這正是把代碼表抽成 CSV 的目的。

### 種子資料寫進資料庫

`scripts/import_seed.py` + `tests/test_import_seed.py`（新增 18 個測試，累計 62 個）。

匯入結果：代碼表 1,023／358／6,222，GWP 4，公告版本 1，燃料係數 12，熱值 6，電力 2。

**兩個驗收條件，都寫成測試了：**

1. **跑兩次結果相同**（`test_running_twice_changes_nothing`）。不只驗筆數 —— 筆數
   相同也可能是「刪掉再新增」。第二次跑的 tally 必須全部落在 unchanged。
2. **從資料庫重算 12 個係數，與試算表 U 欄一致**（`test_factors_survive_the_round_trip`，
   最大差 4.4e-16）。匯入時欄位對錯（CO2 寫進 CH4 欄）不會有任何錯誤訊息，資料庫
   看起來完全正常，只是每個數字都錯。這是唯一擋得住的東西。實際做過突變測試確認
   它會紅。

**幾個決定：**

- **upsert 而不是「刪掉重建」。** 刪掉重建會讓已算好的 `EmissionResult` 外鍵指向
  消失的列，而且每跑一次 id 就換一輪 —— 快照就不再是快照。
- **公告出處從試算表讀，不手抄。** 「資料來源與限制」B4 寫著「環境部 113年2月5日
  環部授氣字第1139101231號公告」，`FactorSet.version` 直接用公告文號。文號本來就是
  那一版的唯一識別，改版自然變成新的一筆，符合 models.py 的「改版＝新增一筆，永不
  UPDATE 舊的」。為此在 `seed.py` 加了 `FactorSource`。
- **`org_id` 有值的熱值絕不覆蓋。** 那是事業自填的資料，models.py 明講熱值是唯一
  「使用者可以合法覆寫官方值」的東西。有測試守著。
- **整批同一個 transaction，任一步失敗全部回滾。** 匯入到一半的資料庫比空的危險，
  因為它看起來是能用的。

### 服務層前的三件清理

**1. B001 燃氣台爐（改 v5 表三）**

S04 廚房天然氣爐具的設備代碼 `0020 燃氣鍋爐`（工業鍋爐）改為 `B001 燃氣台爐`，
`代碼表` 分頁的設備清單同步替換，備註 R7 改寫（原文「設備代碼無『瓦斯爐』，暫歸
燃氣鍋爐」已不成立）。

改法照舊：**只換 `xl/sharedStrings.xml` 裡的字串與 `sheet2.xml` 的公式快取值，
其餘 zip entry 原樣複製。** 用 openpyxl 重寫會丟掉公式快取值。

剛好只要動三個 sharedString：`[105] "0020"` 被 表三 G7 與 代碼表 D6 共用，改一處
兩邊都會動；`[350] "燃氣鍋爐"`；`[109]` 備註。另外 表三 H7 是
`INDEX/MATCH` 公式，它的快取值也要手動更新成「燃氣台爐」。

改完逐格驗過：**7.531736 與所有公式快取值完好**。

**2. 時間戳記改成帶時區**

`dt.datetime.utcnow()` 不只是 deprecated，它回傳的是「UTC 的時刻卻沒有時區標記」，
跟本地時間一比就靜靜差 8 小時。

改的過程發現一個 SQLite 特有的坑：**`DateTime(timezone=True)` 在 SQLite 上是空頭
支票** —— 寫進去帶時區，讀出來 `tzinfo` 是 `None`。值沒錯，但型別跟 PostgreSQL
不一樣，`db.py` 說的「換資料庫只要改 `CARBON_DB_URL`」就不成立。

所以包了一層 `UtcDateTime`（TypeDecorator）：一律以 UTC 儲存，讀出來一律帶時區，
naive datetime 直接擋在寫入端。`tests/test_models.py` 新增 4 個測試守著（累計 66 個）。

**3. 固定／移動燃燒依表三拆分（服務層規格）**

試算表把範疇一全部掛在「固定燃燒」，表八 B11 自己註明「本試算表未依設備別拆分，
程式版應由表三排放源清冊的…」。程式版要做對。

**依 `EmissionSource.emission_type`（表三「排放型式」欄）拆，不是依燃料種類。**
理由：柴油在示範案例裡同時出現在固定（S05 備用發電機）與移動（公務車）兩邊，
依燃料猜一定猜錯。

已用試算表資料驗算過，服務層的目標數字：

| | tCO2e |
|---|---:|
| 範疇一 固定燃燒 | 0.590866 |
| 範疇一 移動燃燒 | 0.214352 |
| **範疇一 小計** | **0.805218** ← 對得回表八 B12 |
| 範疇二 外購電力 | 6.726519 |
| **總計** | **7.531736** |

### 示範資料與服務層

`scripts/load_demo.py` + `app/service.py` + `scripts/calc_demo.py`
+ `tests/test_service.py`（新增 20 個測試，累計 86 個）。

**從資料庫算出 7.531736 tCO2e，與試算表表八一致。** 每個分項也都對得上：

| | 程式（從 DB） | 試算表 |
|---|---:|---:|
| 範疇一 固定燃燒 | 0.590866 | （試算表未拆） |
| 範疇一 移動燃燒 | 0.214352 | （試算表未拆） |
| 範疇一 小計 | 0.805218 | 0.805218 |
| 範疇二 外購電力 | 6.726519 | 6.726519 |
| **總計** | **7.531736** | **7.531736** |
| 推估排放量占比 | 40.9488% | 40.9488% |

**兩處程式版比試算表準：**

1. **固定／移動燃燒有拆開**（試算表表八 B11 自己註明做不到）。
2. **氣體別是真的拆**。試算表把 100% 都算成 CO2，實際上燃料燃燒會排 CH4 與 N2O
   （每單位係數裡本來就含它們）。程式算出 CO2 99.88%／CH4 0.03%／N2O 0.09%。
   總量不變，拆的是同一筆錢怎麼分。

**示範資料走 C：獨立的 `load_demo.py`。** `import_seed.py` 只放官方參考資料，
示範小吃店是假資料，不該混進每個人的資料庫。`--clear` 可移除。兩支都可重複執行。

**服務層強制的兩件事**（資料庫沒有這些約束）：

- **推估必須填依據。** 而且跨期分攤後才知道一筆資料是不是推估 —— 那是計算過程
  產生的事實，寫入當下還不知道，所以只能由這一層擋。標「實測」卻跨年度的也擋。
- **不支援的排放型式報錯，不算成 0。** 製程、逸散、外購蒸汽目前不做，但算成 0
  會讓報告少一截卻沒有任何提示。

**做過突變測試**，四種典型的安靜錯誤都被抓到：分攤失效、固定/移動不拆、
範疇一二判斷反轉、推估檢查被拿掉。

**讀取「活動數據登錄」的坑比先前記的更深。** 那張表有三種列：

```
A 有序號、B 與 G 有值    真正的單據         第 7~11 列
A 有序號、B 與 G 空白    預先編號的空白範本  第 12~30 列
A 空白、B 有名稱         分頁自己的小計列    第 33~35 列
```

只看「A 是不是數字」會把空白範本讀成一堆空記錄；只看「B 有沒有值」會把小計列
讀進來，**總量直接翻倍而且沒有錯誤訊息**。兩個條件都要。表三則是另一種形狀
（尾端有一列說明文字），改用「遇到整列空白就停」，再回頭檢查後面還有沒有漏網的
排放源 —— 否則清單中間被插一列空白就會靜靜截斷。

### HTTP API

`app/api.py` + `tests/test_api.py`（新增 24 個測試，累計 110 個）。
（後來做介面時又補了 5 個，見下一節。）

```bash
uvicorn app.api:app --reload      # → http://127.0.0.1:8000/docs
```

10 個端點。這一層刻意很薄 —— 所有判斷都在 `service.py`，API 只做三件事：
把請求轉成服務層呼叫、把 ORM 轉成 JSON、把例外轉成 HTTP 回應。裡面沒有一行計算。

**三個設計決定：**

1. **GET 不寫資料庫。** 原本想把「算」跟「看」合在 `GET /summary` 裡，很方便 ——
   但那表示每次重新整理報表頁都在改 `calculated_at`，而那正是稽核要看的欄位。
   拆成 `POST /calculate`（算、寫）與 `GET /summary`（只讀）。為此把
   `calculate_year` 拆出 `stored_summary`，兩個獨立的公開函式而不是一個帶旗標的
   —— 呼叫端最該一眼看出來的就是「這個會不會寫資料庫」。

   附帶要處理的：還沒算過時總量是 0，跟「真的排放 0」長得一樣。加了
   `uncalculated_count` 讓兩者分得開。

2. **錯誤帶機器可讀的 `code`。** HTTP 狀態碼分不出「係數還沒公告」跟「係數編號
   打錯」，但前端該做的事完全不同 —— 一個顯示「等公告」，一個要跳到表三讓使用者
   改。所以 `ServiceError` 加了 `code` 類別屬性，並拆出 `FactorNotPublishedError`
   （刻意繼承 `FactorNotFoundError`，既有的 except 照樣接得住）與
   `UnsupportedEmissionTypeError`。

3. **新增單據就立刻計算，失敗則整筆回滾。** 推估沒填依據、跨年帳單標成實測、
   係數查不到 —— 要在使用者還看著表單時就講，而不是等按下「產生報告」才一次爆出。
   而留下一筆算不出結果的活動數據比擋下來更糟：它會出現在清冊上，卻永遠不進總量。

**驗證方式**：除了 `TestClient`，也真的用 `uvicorn` 跑起來、用 `urllib` 打過一輪，
確認 Swagger UI 出得來、中文查詢參數（URL 編碼後）搜得到、422 那一筆確實沒在真的
`carbon.db` 留下殘骸。做過突變測試，四個都抓到：GET 改成會寫入、失敗不回滾、
搜尋不回報截斷、錯誤 code 退回泛用值。

**`requirements.txt` 改成按層次分組。** 計算引擎只要 pytest、種子資料加 openpyxl、
資料庫加 SQLAlchemy、API 才加 fastapi。這個分層是這個專案刻意維持的性質，清單
本身就是「你怎麼確保計算正確」的一部分答案。

（`httpx2` 是 starlette 新版 `TestClient` 要的；裝舊的 `httpx` 也能跑，只是每次
測試都會噴 deprecation 警告。）

### 使用者介面

`app/static/index.html`（單一檔案）+ `GET /` + `tests/test_api.py` 補 5 個測試
（累計 115 個）。

```bash
uvicorn app.api:app --reload      # → http://127.0.0.1:8000/
```

三個畫面：表三排放源清冊、登錄活動數據、表八彙總。

**不用 React／Vue，單一 HTML 檔內嵌 CSS 與 JS。** 這個作品的重點在盤查邏輯與資料
正確性，不在前端工程；多一個 build step 就多一個「在我電腦上跑得起來」的理由。
而且 demo 影片要能只靠一個網址播完，也要能離線播 —— 有測試
（`test_ui_is_self_contained`）擋住任何外部 CDN 引用。

**做的過程補了一個 API 缺口**：`SourceOut` 原本只回代碼不回名稱，但官方表三本來就是
「代碼＋名稱」兩欄並列。畫面上顯示 `9999` 沒有人看得懂，而看不懂的欄位使用者就會
亂填。加了 `process_name`／`equipment_name`／`material_name`，並且**一次查完不在
迴圈裡逐筆查** —— 5 個排放源看不出差別，真的事業有幾十個時那會變成上百次查詢。
有測試數 SQL 次數擋住這件事。

**推估依據前端擋一次、後端擋一次。** 前端是為了體驗（選「推估」時欄位標題即時變成
必填），後端才是正確性的來源 —— 只靠前端擋的規則，任何人用 curl 就繞過去了。
兩邊都實測過。

**驗證**：用瀏覽器實際走過三個畫面，零 console 錯誤；表八顯示 7.531736；登錄流程
從「推估沒填依據被前端擋」到「填了之後成功並顯示完整 calc_trace」都跑過。突變測試
三個都抓到：N+1 查詢、引用外部 CDN、代碼名稱不回傳。

順帶一提，`uvicorn --reload` 在這台機器上偵測到 `api.py` 變更後**重啟卡住**
（只印 Reloading 就沒下文，舊 process 繼續服務）。手動重啟就好，但改完 API 發現
行為沒變時，先確認它真的重啟了。

---

## 這次確認的四件事

### 1. 用自己的購買發票替代真實單據 — 可行，但看清楚它替代掉什麼

加油發票只蓋到範疇一的移動燃燒。在 v5 的示範案例裡，**外購電力佔 6.73 of 7.53 tCO2e，也就是 89%**，而且電費單撐著技術上最漂亮的一段：雙月期帳單跨年度、依 45/62 天分攤、分攤後標記推估。

**可行的組合**：盤查對象改成自家／租屋處，電費單 + 加油發票 + 瓦斯帳單一次到位，全部是真的。

**要誠實寫進報告的**：家戶不是「事業」，官方表一的管制編號、統一編號、行業別代碼填不出來。定位改寫成「以家戶作為方法驗證對象，工具本身仍對齊事業清冊格式」。講清楚就沒事，含糊帶過才有問題。

**不能替代的**：使用者測試。資料來源可以是自己，但操作測試必須是外人——自己測自己一定順，那個數據沒有意義。

### 2. 用自己的載具 — 可行，而且是升級

財政部電子發票整合服務平台可匯出消費明細 CSV，今天就能拿到真實資料，整段 QR code 解碼可以不寫。

**要先驗證**（五分鐘的事，但它決定要不要寫回推邏輯）：載具明細裡的加油發票**有沒有品項與公升數**。很多加油站只開「油料 + 金額」。若只有金額就要走油價回推，那是推估值——v5 活動數據登錄第 4 筆就是這個情境（發票僅 1,500 元，以 30.5 元/公升回推）。

⚠️ **載具消費明細是完整的個人消費紀錄，repo 是 public，絕對不能進去。** `.gitignore` 已擋 `載具*.csv`、`消費明細*.csv`、`發票明細*.csv`、`data/private/`。測試資料一律自編假的。

### 3. 沒有指導老師 — 這一項不再是阻塞項

範疇一二的界定已經自洽：v5 表二寫了三條排除理由（製程、冷媒、化糞池），「資料來源與限制」第 7 條揭露了不含範疇三。沒有人點頭，這個界定一樣站得住。

原計畫表把「跟老師確認範疇界定」列為阻塞 Y，該項作廢。

### 4. 官方表單 — 檔案正確（見上一節）

---

## 路線圖調整（取代計畫表的 W1~W9）

自主練習沒有口試，時間重新分配：**W9 收尾大幅縮減，省下的時間給 W4 電費單解析**，因為後者是作品集裡看得見的東西，簡報不是。

| 順序 | 做什麼 | 原計畫 | 調整後 |
|---|---|---|---|
| 1 | 代碼表抽成 CSV | W2② | ✅ 完成，7,603 筆在 `data/codes/` |
| 1.5 | 種子資料寫進資料庫 | W2① | ✅ 完成，`scripts/import_seed.py` |
| 2 | 服務層：資料庫 ↔ 計算引擎 | W2④ | ✅ 完成，從 DB 算出 7.531736 |
| 3 | API | W2③ | ✅ 完成，10 個端點 |
| 4 | 最小可用介面：輸入到看見表八 | W3 | ✅ 完成，三個畫面 |
| 5 | 報表與 Excel 匯出 | W7 | **現在做**，核心 |
| 6 | 電費單解析 | W4，可砍 | **升級為值得做**（省下的時間給它） |
| 7 | 發票載具 CSV 匯入 | W5，可砍 | 用自己的載具，成本降低 |
| 8 | 使用者測試 | W8，不可砍 | 保留，但找同學／家人即可，不需實際店家 |
| 9 | 書面報告 → 改成 README + demo 影片 | W9 | **大幅縮減**，簡報與演練取消 |
| — | 係數自動對應 | W6，第一個砍 | 維持放棄，用下拉選單人工指定 |

**兩件仍然不要砍**：使用者測試（沒有真人操作過的工具，problem 都藏著）、demo 影片（能獨立播放，不依賴環境）。

### 順序有一個變動，理由要記住

原本規劃「官方代碼表匯入」排最後，因為當時檔案還沒有。**現在要提前到最前面**——`PublishedFactor.material_code` 是指向 `MaterialCode` 的外鍵，代碼表沒先進資料庫，係數就沒有東西可以指。

---

## 下一步（可直接開工）

~~代碼表 CSV~~ ✅　~~`import_seed.py`~~ ✅　~~服務層~~ ✅　~~API~~ ✅　~~介面~~ ✅

**一條龍已經打通**：官方 .ods → CSV → 資料庫 → 服務層 → API → 瀏覽器，
`http://127.0.0.1:8000/` 打開就能操作，表八顯示 7.531736。

接下來兩個方向，建議照這個順序：

### 1. 報表匯出（W7）

現在數字都算得出來，但**帶不走**。要能產出交得出去的東西：

- **匯出成官方清冊格式的 xlsx** —— 表二／表三／表四／表八填進官方表單。
  `EmissionResult` 的快照欄位讓每個數字都交代得出來源，這是這個工具比手填強的地方。
- 注意 openpyxl 寫入既有 xlsx 會丟掉公式快取值（這個坑踩過兩次了），
  匯出用的範本要嘛不含公式，要嘛沿用「只換 sharedStrings」那套。

### 2. 使用者測試（W8，不可砍）

介面有了才測得了。找同學或家人，**不要自己測** —— 自己測自己一定順，那個數據沒有
意義。重點觀察：

- 看不看得懂「排放源」「活動數據」這些官方用語
- 推估依據那個必填欄位，會不會隨便打一個字就過（如果會，那欄位的說明要改）
- 完整性檢查的警告，看不看得懂該做什麼

### 還沒解決的兩件事（不擋路，但報告要寫）

- **熱值假設** —— 天然氣用 NG1(消費面) 8,107，四個數值最大差 48%。見「待決事項」。
- **示範資料只涵蓋 1~4 月** —— 7.531736 是不完整年度的數字。完整性檢查照實噴警告，
  這是刻意保留的，正好展示功能。

---

## 待決事項

- **熱值假設**：天然氣目前用 NG1(消費面) 8,107 kcal/m³。熱值表四個天然氣數值最大差 48%，實務上應改用瓦斯公司費率公告的當期熱值。這是最大的未決假設，v5「資料來源與限制」第 3 條已揭露。
- ~~**`HeatingValue.factor_key` 的鍵怎麼定**~~ → **已解決。** 依 `material_code` 去重，
  12 個燃料列收斂成 6 筆；`factor_key` 也填 `material_code`。實際查過六組全部一致
  （汽油四列都是 7,520 kcal/公升）。一致性檢查照樣寫進 `collapse_heating_values`，
  不一致就報錯 —— 現在沒撞到不代表以後不會。自然鍵是 `(material_code, org_id)`，
  `org_id` 為 None 才是系統預設，事業自填的不會被匯入蓋掉。
- ~~**代碼表對不上小店實況**~~ → **已修正**（2026-08-12）。詳見下方「B001 燃氣台爐」。
- ~~**`models.py` 用 `dt.datetime.utcnow`**~~ → **已修正**（2026-08-12）。詳見下方「時間戳記」。

- **示範資料怎麼進資料庫** —— 已決定走 **C：獨立的 `scripts/load_demo.py`**，明確
  opt-in。`import_seed.py` 只放官方參考資料（任何使用者都需要），示範小吃店是假資料
  （電號、車牌都是編的），不該混進去。讀取邏輯擴充在 `seed.py`，測試 fixture 與
  `load_demo.py` 共用同一份，只寫一次。**尚未實作**，服務層要靠它才驗得起來。

  實作時的坑：「活動數據登錄」第 33~35 列是該分頁自己的小計列（範疇一／範疇二／
  合計），B 欄與 V 欄都有值，天真地掃全部列會把小計當成資料吃進去。要依 A 欄序號
  是不是數字來判斷。
